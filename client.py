...
parse = argparse.ArgumentParser(description="clients/bots for the chat")  # Create the parser
parse.add_argument("-d", "--debug", help="prints debug messages", action="store_true")  # Add the debug argument
parse.add_argument("-i", "--ip", help="ip address of the server", default="", required=True)  # Add the ip argument
...
SERVER = args["ip"]  # The IP address of the server
...
def response(verb):  # Formats a response
    ...
    if verb == Undefined:  # If the verb is undefined
        ...
        response = random.choice(responses)  # Choose a random response from the response list
        response = response.format(verb=verb)  # Format the response
        debug(f"Responding with {response}")  # Log the response
...
def setup():  # Setup the client. This is called when the client is started
    ...
    print("3: Eric")
    print("4: Jonathan")
    userinput = input("Choose bot: ")
    if(str(userinput) == "1"): # If the input is 1 then start Alice
        debug(f"{userinput} is input. Alice is starting")
        bot("Alice")
    elif(str(userinput) == "2"): # If the input is 2 then start Bob
        debug(f"{userinput} is input. Bob is starting")
        bot("Bob")
    elif(str(userinput) == "3"): # If the input is 3 then start Eric
        debug(f"{userinput} is input. Eric is starting")
        bot("Eric")
    elif(str(userinput) == "4"): # If the input is 4 then start Jonathan
        debug(f"{userinput} is input. Jonathan is starting")
        bot("Jonathan")
    else:
        print("You have to choose a bot. Type a number between 1 and 4")
        debug(f"{userinput} is input. No bot is starting")
        setup()
        os.system("CLS")
